...
| 6 | `proguard-rules.pro` + Gradle wiring; `backup_rules.xml` + `data_extraction_rules.xml` | [x] | Verified by CI on PR #1: the release AAB builds with R8 and passes the 16 KB alignment check. |
| 7 | Pin Flutter (`.fvmrc`, CI `flutter-version`); `targetSdk = 36` explicit | [x] | Pinned 3.47.2; pubspec.lock and analysis_options.yaml refreshed under that SDK. Owner: run `fvm install` or match locally. First CI run showed Flutter 3.47 refuses Kotlin < 2.2.20 → KGP bumped to 2.2.20. Gradle 8.14.3 / AGP 8.11.1 still build but Flutter warns they will be dropped; the 3.47 template is Gradle 9.3.1 / AGP 9.1.0 / KGP 2.4.0. Upgrading AGP to 9 flips `android.newDsl`/built-in Kotlin and should be done with a local build, not blind. |
...
| 10 | CI: signed Android release build (skips signing when secrets absent) + iOS compile job | [x] | First run on PR #1 caught two real problems the container could not: Kotlin plugin below Flutter 3.47's minimum, and six Swift call sites passing `LocalizedStringResource` where the runner's SDK wants a string (now `String(localized:)`); iOS deployment target raised 13.0 → 15.0 (Flutter 3.47 minimum); and the release bundler rejects the runtime `IconData(codePoint)` used for account avatars under icon tree-shaking → `lib/data/account_icons.dart` resolves code points to the const picker icons instead. Three jobs: check (lockfile, generated-code drift, analyze --fatal-infos, tests), android (AAB + 16 KB zipalign check + artifacts), ios (build --no-codesign + asserts Release plist has no Bonjour keys). Owner adds secrets `UPLOAD_KEYSTORE_B64`, `KEYSTORE_PASSWORD`, `KEY_ALIAS`, `KEY_PASSWORD`. Green on PR #1 (run 109): check, Android release bundle, iOS build. |
...
- 2026-09-05 · Phase 3 steps 1, 2, 6, 8 complete; 3 and 5 partial (see notes); 4 and 7 wait for a device pass. 153 tests, analyzer clean under strict lints. Owner answered: PR opened as https://github.com/vukbundalo/platrare/pull/1 (session subscribed to its CI); risky Phase 3 refactors wait for the device pass; Phase 4 = own E2E-encrypted backend (SYNC_DESIGN.md).
- 2026-09-05 · PR #1 CI green on all three jobs after four real fixes only a build could find (Kotlin plugin floor, Swift string sites, iOS 15 deployment target, icon tree-shaking). PR is mergeable; merging is the owner's call.
