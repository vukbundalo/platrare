...
| 5 | Deduplicate account forms, currency pickers (one done), date-filter machines, discard dialogs | [~] | Done: `CurrencyPickerSheet` (Settings), `PeriodFilter` (`lib/utils/period_filter.dart`) replaces the three date-filter state machines in Track, Plan and account history (−393/+66 lines; test `period_filter_test.dart`). Remaining: the two 700-line account forms, the account-form currency picker copy, the four discard dialogs. |
| 6 | Enums for section / stats / period / type-group state | [~] | `PeriodUnit`, `TxTypeGroup` (models/account.dart; replaces three `_kType*`/`_inGroup` copies), `_ReviewSection`, `_StatsMode` done. Remaining: `_spendingMonths` magic ints in Review. |
